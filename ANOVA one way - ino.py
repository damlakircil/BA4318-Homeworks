...
#One way ANOVA on the type of sign
...
names = ['K1', 'K2', 'K3', 'K4', 'İ1']
score = 'Skor'
score_diff = construct(df, names, score)
signtype = df['Skor']

dfimp = pd.DataFrame({'Skor':score_diff, 'İno':signtype})
summary = rp.summary_cont(dfimp['Skor'].groupby(dfimp['İno']))
print (summary)
corr = dfimp.corr()
print(corr)
y = dfimp['Skor']
...
summary2 = rp.summary_cont(dfimp_last['Skor'].groupby(dfimp_last['İno']))
print (summary2)
...
set1 = dfimp['Skor'][ dfimp['İno'] == 0 ]
set2 = dfimp['Skor'][ dfimp['İno'] == 1 ]
...
if p < plevel and F > Flevel :
    ...
    print("Variance is explained by the sign type factor.")
else:
    print("Variance is not explained by the sign type factor.")
