...
#One way ANOVA on the bone thickness
...
names = ['İno', 'K1', 'K2', 'K3', 'İ1']
score = 'Skor'
score_diff = construct(df, names, score)
bones = df['Skor']

dfimp = pd.DataFrame({'Skor':score_diff, 'K4':bones})
summary = rp.summary_cont(dfimp['Skor'].groupby(dfimp['K4']))
print (summary)
corr = dfimp.corr()
print(corr)
y = dfimp['Skor']
...
summary2 = rp.summary_cont(dfimp_last['Skor'].groupby(dfimp_last['K4']))
print (summary2)
...
set1 = dfimp['Skor'][ dfimp['K4'] == 0 ]
set2 = dfimp['Skor'][ dfimp['K4'] == 1 ]
...
if p < plevel and F > Flevel :
    ...
    print("Variance is explained by the bone thickness factor.")
else:
    print("Variance is not explained by the bone thickness factor.")